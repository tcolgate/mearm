...
func (p *PWMPin) SetDutyCycle(duty uint32) (err error) {
	...
	if val < 0.05 {
        ...
        valStr := fmt.Sprintf("P1-%v=%2.0f%%\n", p.pin, val * 100)
        fmt.Printf("val[%v]: %v", p.pin, valStr)
	return p.piBlaster(valStr)
}

func (p *PWMPin) piBlaster(data string) (err error) {
	fi, err := sysfs.OpenFile("/dev/servoblaster", os.O_WRONLY|os.O_APPEND, 0644)
	defer fi.Close()
